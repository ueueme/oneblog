    ...
    test('should parse headers', () => {
      const parsed = parseHeaders(
        'Content-Type: application/json\r\n' +
          ...
          'Date: Tue, 21 May 2023 09:23:44 GMT\r\n' +
          ':aa\r\n' +
          'key:'
      )
      ...
      expect(parsed['date']).toBe('Tue, 21 May 2023 09:23:44 GMT')
...
运行后，我们会发现 `parseHeaders` 测试组的 `should parse headers` 测试没通过，`expect(parsed['date']).toBe('Tue, 21 May 2023 09:23:44 GMT')` 我们期望解析后的 `date` 字段是 `Tue, 21 May 2023 09:23:44 GMT`，而实际的值是 `Tue, 21 May 2023 09`。
